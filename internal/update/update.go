...
func Check(currentVersion string) (*ReleaseInfo, error) {
	...
	req, err := http.NewRequest("GET", "https://api.github.com/repos/jimbarrett/archivary/releases/latest", nil)
	if err != nil {
		return nil, fmt.Errorf("creating request: %w", err)
	}
	req.Header.Set("User-Agent", "archivary-updater")
...
func Apply(downloadURL string) error {
	...
	tmpFile, err := os.CreateTemp(dir, "archivary-update-*")
	if err != nil {
...
// GoReleaser's {{ .Version }} strips the "v" prefix, so asset names use bare versions.
func AssetName(version string) string {
	v := strings.TrimPrefix(version, "v")
	return fmt.Sprintf("archivary_%s_%s_%s", v, runtime.GOOS, runtime.GOARCH)
}
...
// It tests by creating and removing a temp file in the same directory, since os.Rename
// (used by Apply) works on running executables but os.O_WRONLY does not (ETXTBSY on Linux).
func CanWriteBinary() (string, bool) {
	...
	dir := filepath.Dir(execPath)
	f, err := os.CreateTemp(dir, ".archivary-write-test-*")
	if err != nil {
		return execPath, false
	}
	name := f.Name()
...
func ManualUpdateCommand(downloadURL, binaryPath string) string {
	return fmt.Sprintf("curl -L %s -o /tmp/archivary && chmod +x /tmp/archivary && sudo mv /tmp/archivary %s", downloadURL, binaryPath)
}
